...
import { EApprovalNotificationRow } from "@/components/e-approval/e-approval-notification-row";
import { Button } from "@/components/ui/button";
import {
  useEApprovalNotificationUnreadCount,
  useEApprovalNotifications,
} from "@/hooks/use-e-approval-notifications";
import {
  markAllEApprovalNotificationsRead,
  markEApprovalNotificationRead,
} from "@/lib/api/modules/e-approval-api";
import type { EApprovalNotificationRow as EApprovalNotificationRowType } from "@/modules/e-approval/types";
import {
  countUnreadByTab,
  filterNotificationsByTab,
  type EApprovalNotificationTab,
...
const TABS: { id: EApprovalNotificationTab; label: string }[] = [
  { id: "action", label: "Action required" },
  { id: "update", label: "Updates" },
  { id: "all", label: "All" },
];

export function EApprovalNotificationBell({ enabled = true }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState<EApprovalNotificationTab>("action");
  const queryClient = useQueryClient();
  const unreadQuery = useEApprovalNotificationUnreadCount(enabled);
  const listQuery = useEApprovalNotifications(enabled && open);
  ...
  const markReadMutation = useMutation({
    mutationFn: markEApprovalNotificationRead,
    onSuccess: invalidateNotifications,
  });

  const markAllReadMutation = useMutation({
    mutationFn: () => markAllEApprovalNotificationsRead(tab === "all" ? undefined : tab),
    onSuccess: invalidateNotifications,
  });

  const handleNavigate = (notification: EApprovalNotificationRowType) => {
        ...
        <div className="absolute right-0 top-full z-50 mt-2 w-[min(26rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-border bg-card shadow-lg">
          ...
          <div className="flex items-center gap-1 border-b border-border px-3 py-2">
            ...
            <Button
              ...
              render={<Link href="/e-approval/settings" onClick={() => setOpen(false)} />}
            >
...
export function EApprovalNavBadge({ count }: { count: number }) {
  if (count <= 0) {
    return null;
  }
