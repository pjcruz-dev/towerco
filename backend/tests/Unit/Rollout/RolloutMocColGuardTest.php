...
final class RolloutMocColGuardTest extends TestCase
{
    ...
    public function test_moc_col_blocked_until_pre_assessment_passed(): void
    {
        $program = $this->seedProgram(preAssessmentGate: 'pending', mocGate: 'pending');

        $this->assertFalse(RolloutMocColGuard::isReadyForMocCol($program->fresh(['timelinePhases', 'candidates'])));
        $this->expectException(ValidationException::class);
        RolloutMocColGuard::assertReadyForMocCol($program->fresh(['timelinePhases', 'candidates']));
    }

    public function test_moc_col_ready_after_pre_assessment(): void
    {
        $program = $this->seedProgram(preAssessmentGate: 'passed', mocGate: 'pending');

        $this->assertTrue(RolloutMocColGuard::isReadyForMocCol($program->fresh(['timelinePhases', 'candidates'])));
    }

    public function test_tssr_blocked_until_moc_col_passed(): void
    {
        $program = $this->seedProgram(preAssessmentGate: 'passed', mocGate: 'pending');
    ...
    public function test_tssr_allowed_after_moc_col_passed(): void
    {
        $program = $this->seedProgram(preAssessmentGate: 'passed', mocGate: 'passed');

        RolloutPreAssessmentGuard::assertReadyForTssr($program->fresh(['timelinePhases', 'candidates']));
        $this->assertTrue(true);
    }

    private function seedProgram(string $preAssessmentGate, string $mocGate): RolloutProgram
    {
