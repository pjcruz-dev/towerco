...
final class HelpGuideAdminPublishController extends AbstractApiController
{
    public function __invoke(Request $request, string $slug, HelpGuideService $service): JsonResponse
    {
        ...
        $published = $service->publish($request->user(), $guide);

        return $this->ok($service->asDetail($published));
    }
}
