...
final class HelpGuideAdminUnpublishController extends AbstractApiController
{
    public function __invoke(Request $request, string $slug, HelpGuideService $service): JsonResponse
    {
        ...
        $unpublished = $service->unpublish($request->user(), $guide);

        return $this->ok($service->asDetail($unpublished));
    }
}
