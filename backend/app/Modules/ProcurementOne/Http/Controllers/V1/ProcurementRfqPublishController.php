...
final class ProcurementRfqPublishController extends AbstractApiController
{
    public function __invoke(
        ...
        $validated = $request->validate([
            'bidding_opens_at' => ['sometimes', 'date'],
            'bidding_closes_at' => ['sometimes', 'date', 'after:bidding_opens_at'],
        ]);

        $published = $service->publish($model, $validated, $actor);

        return $this->ok(['rfq' => $registry->toDetailPayload($published)]);
    }
}
