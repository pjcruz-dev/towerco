...
final class TenantUserRevokeSessionsController extends AbstractApiController
{
    public function __invoke(Request $request, TenantUser $user, TenantUserAdminService $service): JsonResponse
    {
        ...
        $actor = $request->user();
        $service->revokeAllSessions($actor, $user);

        return $this->ok(['message' => __('All sessions revoked for this user.')]);
    }
}
