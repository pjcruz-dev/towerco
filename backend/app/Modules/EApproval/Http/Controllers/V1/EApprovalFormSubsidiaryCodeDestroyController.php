...
class EApprovalFormSubsidiaryCodeDestroyController extends AbstractApiController
{
    public function __invoke(
        ...
        $normalized = $pdfLayout->normalizeSubsidiaryCode($code);
        $storage->deleteFormSubsidiaryLogo($form, $normalized);
        $codes = $pdfLayout->unregisterSubsidiaryCode((string) $form->id, $normalized);

        $layout = $pdfLayout->show((string) $form->id);
        $template = is_array($layout['template'] ?? null) ? $layout['template'] : [];

        $audit->log(
            'form_subsidiary_code_removed',
            $form->id,
            $normalized,
            $request->user(),
        );
