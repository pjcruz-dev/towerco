...
class AssistantKnowledgeArchiveController extends AbstractApiController
{
    public function __invoke(Request $request, string $source, TenantKnowledgeService $knowledge): JsonResponse
    {
        ...
        $archived = $knowledge->archive($user, $model);

        return $this->ok($knowledge->asDetail($archived));
    }
}
